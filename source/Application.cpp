...
#pragma endregion
		...
		m_CameraMatrix = glm::mat4(1.f, 0.f, 0.f, 0.f,
			...
			0.f, 0.f, 1.f, 0.f,
			0.f, 0.f, 1.f, 1.f);
		m_cameraPos = glm::vec2(0, 0);
		m_cameraScale = 1.0f;
		int iScreenWidth = 0, iScreenHeight = 0;
		glfwGetWindowSize(m_window, &iScreenWidth, &iScreenHeight);
		m_ProjectionMatrix = glm::ortho((float)iScreenWidth * -0.5f, (float)iScreenWidth * 0.5f, (float)iScreenHeight * -0.5f, (float)iScreenHeight * 0.5f, 0.01f, 1000.f);
		...
		// Get the max amount of texture units
		glGetIntegerv(GL_MAX_TEXTURE_IMAGE_UNITS, &m_maxTextureUnits);
		// Fill the texture slots
		int* activeTextureArray = new int[m_maxTextureUnits];
		for (int i = 0; i < m_maxTextureUnits; i++) {
			activeTextureArray[i] = i;
		}
		GLuint texUniformID = glGetUniformLocation(m_ShaderID, "activeTexture");
		glUniform1iv(texUniformID, m_maxTextureUnits, activeTextureArray);
		delete[] activeTextureArray;
	...
	void Application::run()
	{
		...
		do
		{
			...
			SpriteManager::GetInstance()->Draw(m_ShaderID, m_maxTextureUnits);
...
#pragma endregion
	...
	void Application::SetCameraPos(glm::vec2 pos)
	{
		m_cameraPos = pos;
		m_CameraMatrix = glm::translate(glm::mat4(), glm::vec3(pos, 1.0f));
	}
	void Application::SetCameraScale(float scale)
	{
		m_cameraScale = scale;
		m_CameraMatrix = glm::scale(m_CameraMatrix, glm::vec3(scale, scale, 1.0f));
	}
	void Application::GetCameraPos(glm::vec2 & pos) const
	{
		pos = m_cameraPos;
	}
	void Application::GetCameraScale(float & scale) const
	{
		scale = m_cameraScale;
	}
	//////////////////////////////////////////////////////////////////////////////
